 ...
 * @param result 请求结果
 */
fun <T> MutableResult<ResultState<T>>.paresResult(result: BaseResponse<T>) {
    println("-------$result")
    value = when {
 ...
 * @param result 请求结果
 */
fun <T> MutableResult<ResultState<T>>.paresResult(result: T) {
 ...
 * 异常转换异常处理
 */
fun <T> MutableResult<ResultState<T>>.paresException(e: Throwable) {
