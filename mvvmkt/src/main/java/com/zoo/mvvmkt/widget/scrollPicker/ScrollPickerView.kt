 ...
 * 上下滑动的选择器
 */
class ScrollPickerView(private val context: Context, attrs: AttributeSet?) : View(
    context, attrs
) {
    /**
    ...
    private fun init() {
        ...
        mPaint!!.color = ContextCompat.getColor(context, R.color.date_text_select)
        //第二个paint
        nPaint = Paint(Paint.ANTI_ALIAS_FLAG)
        nPaint!!.style = Paint.Style.FILL
        nPaint!!.textAlign = Align.CENTER
        nPaint!!.color = ContextCompat.getColor(context, R.color.date_text)
    }
    ...
    private fun drawData(canvas: Canvas) {
        ...
        val scale = parabola(mViewHeight / 4.0f, mMoveLen) as Float
    ...
    private fun drawOtherText(canvas: Canvas, position: Int, type: Int) {
        ...
        val scale = parabola(mViewHeight / 4.0f, d) as Float
        val size = (mMaxTextSize - mMinTextSize) * scale + mMinTextSize
